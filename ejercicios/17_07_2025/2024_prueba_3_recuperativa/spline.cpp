...
// Métodos de acceso para los extremos del dominio
double Spline::getXmin() const { return x[0]; }
double Spline::getXmax() const { return x[n]; }
...
Spline::Spline(const VecD& puntosX, const VecD& puntosY, Tipo tipo)
{
    if (puntosX.size() != puntosY.size()) {
        throw std::invalid_argument("Los vectores de entrada deben tener el mismo tamaño.");
    }
    if (puntosX.size() < 2) {
        throw std::invalid_argument("Se requieren al menos dos puntos para el spline.");
    }
    // Verifica que x sea estrictamente creciente
    for (size_t i = 1; i < puntosX.size(); ++i) {
        if (puntosX[i] <= puntosX[i-1]) {
            throw std::invalid_argument("Los valores de x deben ser estrictamente crecientes y no repetidos.");
        }
    }
    x = puntosX;
    y = puntosY;
...
{
    std::ofstream out(archivo);
    double tmin = sx.getXmin();
    double tmax = sx.getXmax();
    double paso = (tmax - tmin) / (cantidadPuntos-1);
    out << "t x_spline y_spline\n";
    for (int i = 0; i < cantidadPuntos; ++i)
    {
        double tval = tmin + i*paso;
        out << tval << " " << sx.evaluar(tval) << " " << sy.evaluar(tval) << std::endl;
    }
    out.close();
}