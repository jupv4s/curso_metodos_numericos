...
// ¡Nuevo include para tu clase Vector!
#include "Vector.h"

// Variable global para el conteo de intercambios de filas en Gauss para el determinante.
// Nota: Una variable global introduce estado compartido, lo cual puede ser problemático en una plantilla.
// Para un diseño más robusto, 'gauss_elimination' podría retornar un par (Matrix, int)
// o el contador podría ser un parámetro de referencia. Por ahora, se mantiene para replicar la lógica existente.
int gauss_swap_count = 0;
...
// Estos operadores + y - ya existen en Vector.h para tu clase Vector.
// Los dejo aquí por si acaso, pero si ya están en Vector.h, son duplicados.
// Se recomienda eliminarlos de aquí y depender de los de Vector.h.
// Los dejo COMENTADOS para evitar conflictos si ya existen en Vector.h.

/*
template <class T>
std::vector<T> operator+(const std::vector<T>& v1, const std::vector<T>& v2) {
    if (v1.size() != v2.size()) {
        throw std::invalid_argument("Error: Los vectores deben tener el mismo tamaño para la suma.");
    }
...
std::vector<T> operator-(const std::vector<T>& v1, const std::vector<T>& v2) {
    if (v1.size() != v2.size()) {
        throw std::invalid_argument("Error: Los vectores deben tener el mismo tamaño para la resta.");
    }
    std::vector<T> result(v1.size());
    for (std::size_t i = 0; i < v1.size(); ++i) {
        result[i] = v1[i] - v2[i];
...
// Esta función 'norm2' es para std::vector<T>, no para tu clase Vector<T>.
// Tu clase Vector<T> ya tiene un método 'norm()'.
// Si quieres una función global para tu Vector<T>, deberías usar Vector<T> en la firma.
// La dejo COMENTADA para evitar confusión si se pretende usar el método .norm() de tu clase Vector.
/*
template <class T>
T norm2(const std::vector<T>& v) {
    T result = T(); // Inicializa a 0 para el tipo T.
    ...
    if constexpr (std::is_floating_point_v<T>) {
        ...
        return static_cast<T>(std::sqrt(static_cast<double>(result)));
    }
}
*/
